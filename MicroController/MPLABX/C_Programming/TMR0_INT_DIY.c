/*
* This program configures the PIC18 to use Timer0 with interrupts to generate a 
* periodic timing signal. The timer is driven by the system clock (FOSC/4) 
* with a prescaler, creating an interrupt approximately every 1.3 ms. 
* Each time the interrupt occurs, the program updates a counter and toggles 
* an output (such as an LED), producing a visible blinking effect.
* In short, the program demonstrates how to:
*         Use the internal/system clock as a timing source
*         Configure Timer0 in 8-bit mode with a prescaler
*         Generate periodic interrupts
*         Use those interrupts to control output behavior (e.g., LED blinking)
*
...
void TMR0_Initialize(void)
{
    ...
    // Set T0CS FOSC/4; T0CKPS 1:256; T0ASYNC synchronised; 
    ?????

    // Set TMR0H ;  The total division will be (FOSC/4)/(4+1)x256)))
    ????? 

    // TMR0L 0; 
    TMR0L = 0x00;

    // Make sure you Clear Interrupt flag before enabling the interrupt
    ????
...
{
    T0CON0=0x00; // stop the timer & do what you have to do - clear it! 
...
{
    // Unhandled interrupts go here - leave this alone
}
